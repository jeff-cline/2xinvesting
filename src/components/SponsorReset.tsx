...
export default function SponsorReset() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    const r = await fetch("/api/sponsor/password", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ password: pw }) }).then((x) => x.json()).catch(() => null);
    setBusy(false);
    if (r?.ok) location.href = "/portal"; else setErr(r?.error || "Could not update password.");
  }
  return (
    <form onSubmit={submit} className="form-wrap" style={{ maxWidth: 420 }}>
      <h1 style={{ fontSize: 28, marginBottom: 8 }}>Set your password</h1>
      <p className="form-note" style={{ marginBottom: 20 }}>You&rsquo;re signed in with the temporary password. Choose a permanent one.</p>
